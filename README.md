...
💡 You can use this repo to preview changes to Style Spec content in the `mapbox-gl-js` submodule. To do this, make your changes to files in the `node_modules` folder, save the files, and then use `npm start` to start the server and view the site in your web browser. Note that you will not be able to commit your changes to files in the `node_modules` folder. You must make your *actual* changes to Style Spec content in the [mapbox-gl-js](https://github.com/mapbox/mapbox-gl-js) repo.
